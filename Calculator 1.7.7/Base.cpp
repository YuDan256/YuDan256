...
void Base::doBase() {
	...
	cout << "Enter n to determine the n - base system:" << endl;
	cin >> _b;
	try {
		b = stoull(_b);
	}
	...
	if (b < 2) {
		cout << "The base cannot be less than 2." << endl;
		doBase();
		return;
	}
	else {
		...
		Base b1, b2;
		string d1, d2;
		cout << "Enter the first number in this numeral system:";
		cin.ignore(numeric_limits<streamsize>::max(), '\n');
		getline(cin, d1);
		cout << "Enter the second number in this numeral system (if you do not need it, enter 0):";
		getline(cin, d2);
		try {
			b1 = enterBase(b, d1);
			b2 = enterBase(b, d2);
		...
		if (b == 2)doBase1(b1, b2);
		else doBase2(b, b1, b2);
	}
	return;
}

//for binary
void Base::doBase1(Base& b1, Base& b2) {
	ull n = 0;
	string d;
	...
	while (1) {
		bool quit = 0, invalidSave = 0;
		cout << "Choose an operation:" << endl
			<< "A - addition (2)" << endl
			<< "B - subtraction (2)" << endl
			<< "C - multiplication (2)" << endl
			<< "D - division (2)" << endl
			<< "E - print (2*)" << endl
			<< "F - base change (2*)" << endl
			<< "G - compare (2*)" << endl
			<< "H - Bitwise AND (2)" << endl
			<< "I - Bitwise OR (2)" << endl
			<< "J - Bitwise XOR (2)" << endl
			<< "K - Bitwise NOT (1)" << endl
			<< "Q - quit" << endl;
		cin >> operation;
		...
		try {
			...
			case'D':
				...
				break;
			case'E':
				cout << "The numbers you just entered are:" << endl;
				b1.print();
				b2.print();
				invalidSave = 1;
				break;
			case'G':
				if (b1 > b2)cout << "Number 1 is greater than Number 2." << endl;
				else if (b1 < b2)cout << "Number 1 is less than Number 2.";
				else cout << "Number 1 and Number 2 are equal." << endl;
				invalidSave = 1;
				break;
			case'H':
				result = b1 & b2;
				cout << "The result of Bitwise AND is:" << endl;
				result.print();
				break;
			case'I':
				result = b1 | b2;
				cout << "The result of Bitwise OR is:" << endl;
				result.print();
				invalidSave = 1;
				break;
			case'J':
				result = b1 ^ b2;
				cout << "The result of Bitwise XOR is:" << endl;
				result.print();
				break;
			case'K':
				result = ~b1;
				cout << "The result of Bitwise NOT of Number 1 is:" << endl;
				result.print();
				break;
			case'Q':
				quit = 1;
...
//for bases greater than 2
void Base::doBase2(const ull& base, Base& b1, Base& b2) {
	ull n = 0;
	string d;
	bool quit = 0, invalidSave = 0;
	char operation, choice;
