...
void Statistics::doStatistics() {
	...
	while (1) {
		cout << "Choose an operation:" << endl
			<< "S - single-variable statistics" << endl
			<< "B - bivariate statistics" << endl
			<< "Q - quit" << endl;
		cin >> operation;
		switch (operation) {
		case'S':
...
void Statistics::doSingle() {
	...
	Statistics X(x);
	cout << "The data you entered are:" << endl;
	X.print();
	cout << "The statistical results are as follows:" << endl;
	newSingle(X);
}

void Statistics::doBivariate() {
	string expr;
	map<string, double>variables;
	vector<double>x, y;
	double _x, _y;
	variables["PI"] = 3.14159265358979323846264;
	variables["E"] = 2.7182818284590452353602874;
	cout << "Enter the data X (enter 'end' to finish):" << endl;
	while (1) {
		cin >> expr;
		if (expr == "end" && x.size() > 1)break;
		else if (expr == "end") {
	...
	Statistics X(x);
	cout << "Enter the data Y (enter 'end' to finish):" << endl;
	while (1) {
		cin >> expr;
		if (expr == "end" && x.size() > 1)break;
		else if (expr == "end") {
		...
		try { _y = Normal::parsen(expr, variables); }
		catch (const exception& e) {
			cerr << "Error: " << e.what() << endl;
			continue;
		}
	...
	Statistics Y(y);
	cout << "The data X you entered are:" << endl;
	X.print();
	cout << endl;
	cout << "The data Y you entered are:" << endl;
	Y.print();
	cout << endl;
	cout << "The statistical results of X are as follows:" << endl;
	newSingle(X);
	cout << endl;
	cout << "The statistical results of Y are as follows:" << endl;
	newSingle(Y);
	cout << endl;
