...
Vector Vector::input() {
	...
	if (n < 0) {
		std::cout << "The dimension of vector cannot be minus." << endl;
		return input();
	}
	if (n == 0)return Vector(0);
	Vector v(n);
	std::cout << "Enter the components of the vector:" << endl;
	for (int i = 0; i < n; i++) {
		cin >> _a;
		try {
...
void Vector::doVector() {
	...
	Vector v1 = input();
	cout << "Input Vector 2 (if you do not need it, type '0')." << endl;
	Vector v2 = input();
	cout << "Input Vector 3 (if you do not need it, type '0')." << endl;
	Vector v3 = input();
	...
	while (1) {
		...
		cout << "Choose an operation:" << endl
			<< "A - addition (2)" << endl
			<< "B - subtraction (2)" << endl
			<< "C - scalar multiplication (1)" << endl
			<< "D - dot production (2*)" << endl
			<< "E - cross production (2)" << endl
			<< "F - mixed production (3*)" << endl
			<< "G - angle (2*)" << endl
			<< "H - modulus (1*)" << endl
			<< "I - scalar projection (2*)" << endl
			<< "J - projection vector (2)" << endl
			<< "K - parallel (2*)" << endl
			<< "L - vertical (2*)" << endl
			<< "M - parallel vectors division (2*)" << endl
			<< "Q - quit" << endl;
		cin >> operation;
		...
		try {
			...
			case 'A':
				result = v1 + v2;
				cout << "The result of Vector 1 and Vector 2 is:" << endl;
				result.print();
				break;
			case 'B':
				result = v1 - v2;
			...
			case 'C':
				...
				result = v1 * scalar;
				cout << "The result of Vector 1 is:" << endl;
				result.print();
				break;
			case 'D':
				cout << "The dot product of Vector 1 and Vector 2 is:" << endl;
				cout << v1.dot(v2) << endl;
				invalidSave = 1;
				break;
			case 'E':
				result = v1 * v2;
				cout << "The cross product of Vector 1 and Vector 2 is:" << endl;
				result.print();
				break;
			case 'F':
				cout << "The mixed product of Vector 1, Vector 2 and Vector 3 is:" << endl;
				cout << v1.mixedProduct(v2, v3) << endl;
				invalidSave = 1;
				break;
			case 'G':
				cout << "The angle between Vector 1 and Vector 2 is:" << endl;
				cout << v1.angle(v2) << endl;
				invalidSave = 1;
				break;
			case 'H':
				cout << "The modulus of Vector 1 is:" << endl;
				cout << v1.modulus() << endl;
				invalidSave = 1;
				break;
			case 'I':
				cout << "The scalar projection of Vector 1 onto Vector 2 is:" << endl;
				cout << v1.scalarProjection(v2) << endl;
				invalidSave = 1;
				break;
			case 'J':
				result = v1.projectionVector(v2);
				cout << "The projection vector of Vector 1 onto Vector 2 is:" << endl;
				result.print();
				break;
			case 'K':
				cout << (v1.parallel(v2) ? "Vector 1 and Vector 2 are parallel." : "Vector 1 and Vector 2 are not parallel.") << endl;
				invalidSave = 1;
				break;
			case 'L':
				cout << (v1.vertical(v2) ? "Vector 1 and Vector 2 are vertical." : "Vector 1 and Vector 2 are not vertical.") << endl;
				invalidSave = 1;
				break;
			case 'M':
				cout << "The result of Vector 1 and Vector 2 is:" << endl;
		...
		catch (const bool& b) { invalidSave = b; }
		if (quit)break;
		if (invalidSave) {
			cout << "The result cannot be saved." << endl;
			cout << "Do you want to input new Vector 1? (Y/N)" << endl;
			cin >> choice;
			if (choice == 'Y')v1 = input();
			else if (choice == 'N') {}
			else cout << "Invalid Choice." << endl;
			cout << "Do you want to input new Vector 2? (Y/N)" << endl;
			cin >> choice;
			if (choice == 'Y')v2 = input();
			else if (choice == 'N') {}
			else cout << "Invalid Choice." << endl;
			cout << "Do you want to input new Vector 3? (Y/N)" << endl;
			cin >> choice;
			if (choice == 'Y')v3 = input();
			else if (choice == 'N') {}
			else cout << "Invalid Choice." << endl;
		}
		else {
			...
			if (choice == 'N') {
				...
				cout << "Where do you want to save the result? (1/2/3)" << endl;
				cin >> choice;
				if (choice == '1') {
					v1 = result;
					cout << "Do you want to input new Vector 2? (Y/N)" << endl;
